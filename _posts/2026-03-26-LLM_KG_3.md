---
layout: post
title: "第3回：20点からの格闘記 ─ ハルシネーションを封じ込める『逃げ道』の設計"
date: 2026-03-26
...
series_order: 3
description: "AIの『善意の補完』が科学データの『悪意の捏造』に変わる瞬間。20点の赤点から98点の一級燃料へ。作業計画の査読（Meta-Prompting）と物理の檻（PSP）でAIを調教した、泥臭くも知的な格闘の記録。"
published: true
---
...
<div align="center">
<img src="{{ '/assets/img/2026-03-26-LLM_KG3/first_markdown_overall.png' | relative_url }}" width="50%" alt="第1回抽出結果のMarkdownファイル">
</div>
...
致命的欠陥を正すために「どんなVibe(専門知識)をAIに注入すべきか？」それを知るために私が取った作戦は、**AIと「別チャット」で徹底的な原因究明を行うこと**です。そして、明らかになった致命的欠陥の根幹は、「自由過ぎる形式（Markdown）」故に、材料科学では常識的な「物理的な縛り」がほとんど抜け落ちている、ということです。
...
```json
    {
      "@id": "route:Disk_Fabrication_TN_MgO",
      "@type": "aria:ManufacturingRoute",
      "description": "Fabrication route for TN-MgO (Magnesium oxide co-doped with Titanium and Niobium) disks",
      "hasStep": ["step:tn_disk_1", "step:tn_disk_2", "step:tn_disk_3"]
    },
    {
      "@id": "step:tn_disk_1",
      "@type": "process:BallMilling",
      "description": "NOT_STATED",
      "parameters": {
        "raw_materials": ["Magnesium Oxide (MgO)"],
        "additives": [
          { "type": "Titanium Dioxide (TiO2)", "amount": "0.15 at.%" },
          { "type": "Niobium Pentoxide (Nb2O5)", "amount": "0.10 at.%" }
        ],
        "time": "NOT_STATED",
        "speed": "NOT_STATED"
      },
      "post_process_state": "state:tn_mixed_powder",
      "nextStep": "step:tn_disk_2"
    },
    {
      "@id": "state:tn_mixed_powder",
      "@type": "state:PowderMixture",
      "description": "NOT_STATED"
    },
    {
      "@id": "step:tn_disk_2",
      "@type": "process:ColdIsostaticPressing",
      "description": "NOT_STATED",
      "parameters": {
        "pressure": "200 MPa",
        "temperature": "Room Temperature (Implied)"
      },
      "post_process_state": "state:tn_green_disk",
      "nextStep": "step:tn_disk_3"
    },
```

これで完璧かと少し浮かれていましたが、仔細確認した結果、まだいくつか抜け漏れがあり、完成度は90点。ここで妥協しては後の結果への影響が大きいため、もう少しだけ抽出プロンプトの調整を継続しました。

### 3-3. Process粒度の調整、略号展開の厳密化
...
```json
    {
      "@id": "state:final_tim",
      "@type": "state:ThermalInterfaceMaterial",
      "description": "NOT_STATED",
      "hasProperty": [
        {
          "@type": "property:ThermalConductivity",
          "value": "8.0 - 10.0 W/(m·K)"
        },
        {
          "@type": "property:CoolingPerformance",
          "cooling_time_80_to_40C (Simulation)": "2.5 times faster than commercial 3.0 W/(m·K) Al2O3 TIM",
          "cell_temp_after_0.8s (Simulation)": "24.5 °C (from 80°C initial)",
          "is_simulated": true
        }
      ]
    }
